...
2. Ensure that $p_R=p_D$ for all $f$, following Theorem \ref{thm:pairwise} ($p_R$ is the probability that discriminant function $f$ ``favors'' Reserved data while $p_D$ is the probability with which it favors the Defender data).
3. Ensure that $e_R=e_D$, following Theorem \ref{thm:blf} ($e_R$ is the expected value of the loss on Reserved data and $e_D$ on Defender data).
4. Include some randomness in the Defender trainer algorithm, after Theorem \ref{thm:deterministic}.
\end{itemize}
